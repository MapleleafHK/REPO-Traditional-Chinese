...
if exist "REPO-zhHK-v1.2.0 -AutoInstall.zip" (
    echo [執行] 正在解壓縮 REPO-zhHK-v1.2.0 -AutoInstall.zip ...
    powershell -Command "Expand-Archive -Path 'REPO-zhHK-v1.2.0 -AutoInstall.zip' -DestinationPath '.' -Force"
    goto COPY_FILES
) else (
    echo [錯誤] 找不到 .tsv 檔案或 REPO-zhHK-v1.2.0 -AutoInstall.zip。
    goto P_PAUSE
)
