...
if exist "REPO-zhHK-v1.0.0.zip" (
    echo [執行] 正在解壓縮 REPO-zhHK-v1.0.0.zip ...
    powershell -Command "Expand-Archive -Path 'REPO-zhHK-v1.0.0.zip' -DestinationPath '.' -Force"
    goto COPY_FILES
) else (
    echo [錯誤] 找不到 .tsv 檔案或 REPO-zhHK-v1.0.0.zip。
    goto P_PAUSE
)
